...
```{.latex caption="摘要代码框架示例" label=chapters:abstract}
\frontmatter
\pagestyle{main}
...
```{.latex caption="前言后文代码框架示例" label=chapters:frontword：nonhead}
# 主要符号对照表
...
```{.latex caption="正文第一章代码框架示例" label=sec:mainpage:head}
\mainmatter
\pagestyle{main}
...
```{.latex caption="附录代码框架示例" label=sec:appendix:head}
\appendix
...
相关代码为 latex 形式，位于 *./tex/* 文件夹下，请参考[上海交通大学学位论文Latex模板](https://github.com/sjtug/SJTUThesis)。
